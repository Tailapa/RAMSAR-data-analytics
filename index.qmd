...
format:
  html:
    ...
    code-fold: false
    code-tools: false
    embed-resources: true  # This is crucial for offline rendering
    self-contained: true
    fig-height: 6  
    fig-width: 6 
    fig-align: center
...
```{=html}
<iframe 
    src="key_findings.html" 
    id="key_f"
    width="100%" 
    style="border:none; width:100%; height: 500px; min-height: 250px;" 
    frameborder="0">
</iframe>
```
...
<iframe
    ...
    id="met_f"
    width="100%" 
    style="border:none; width:100%; height: 500px; min-height: 300px;" 
    frameborder="0" >
</iframe>
```
...
local_json <- "ramsar_df_India.php"

# Geometric Resilience Logic: Check if local 'Hub' exists or fetch from 'Remote'
if (!file.exists(local_json)) {
  tryCatch({
    # Mimic a browser request to avoid 403/connection errors
    resp <- GET(api_url, user_agent("Mozilla/5.0"), timeout(30))
    writeBin(content(resp, "raw"), local_json)
    message("Data anchored locally from API.")
  }, error = function(e) {
    stop("Failed to connect to the Ecological Pulse-Node (API). Check internet or URL.")
  })
}

# Fetch and parse the data
# Note: Sometimes government APIs require a 'User-Agent' header to allow access
ramsar_json <- fromJSON(local_json)
...
local_file <- "ramsar_data.csv"

# Innovative "Local-First" approach
if (!file.exists(local_file)) {
  # Use a more modern downloader that handles HTTPS/SSL better than basic read.csv
  download.file(csv_url, destfile = local_file, mode = "wb")
}

# Read from the local copy
rsis_df <- read.csv(local_file)
```
...
<style>
        :root {
            --primary: #1A5E5E;      /* Deep Wetland Teal */
            --accent: #E76F51;       /* Risk Coral */
            --surface: #F8FAFA;      /* Geometric Gray */
            --text: #2C3E50;
        }
...
<body>
    ...
    <div id="final-conclusion" class="conclusion-container">
        
        
        <p>
            Overall, India’s 
            Ramsar policy has moved from slow, emblematic protection to rapid, 
            criteria‑rich expansion, but the first four decades largely failed 
            to anticipate how urbanisation, infrastructure and climate pressures 
            would converge on the wetlands of highest ecological value. 
            Therefore, we argue for a decisive shift from nominal 
            designation to risk‑aware governance: rebalancing Ramsar criteria 
            toward under‑represented ecosystem functions, de‑risking mega‑anchors, 
            protecting clusters and corridors, and using the Ramsar Red List as a 
            priority list for funding, enforcement, and Amrit Dharohar interventions 
            so that India’s expanded network translates into durable ecological 
            security rather than a larger, more fragile inventory on paper.
        </p>
        
        <div class="note-text">
            <p><strong>NOTE: </strong>This framework is designed as a data‑driven tool that can be updated 
            each year as new Ramsar sites are designated, criteria are revised, or 
            additional threats are reported, ensuring that India’s wetland policy 
            remains responsive to a changing ecological and institutional landscape.
            </p>
        </div>
