...
ggplot(risk_value_data, aes(x = Diversity_Score, y = Threat_Count)) +
  # Quadrant Shading
  annotate("rect", xmin = 5, xmax = 9, ymin = 10, ymax = max(risk_value_data$Threat_Count), 
           fill = "#E76F51", alpha = 0.2) + # The "Red List" Zone
  # Data Nodes
  ...
  geom_text(data = filter(risk_value_data, Diversity_Score >= 6 & Threat_Count >= 12),
            aes(label = Site.name), vjust = -1, size = 3, family = "mono") +
  # Geometric Styling
  theme_minimal() +
  labs(
    title = "RISK-TO-VALUE MATRIX: THE RED LIST NODES",
    subtitle = "Identifying high-complexity sites under extreme anthropogenic pressure",
    x = "Value Vector (Diversity Score)",
    y = "Risk Vector (Total Threat Count)"
  ) +
  theme(
    panel.grid.minor = element_blank(),
