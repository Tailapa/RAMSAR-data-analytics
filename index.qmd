...
<script>
  window.addEventListener('message', function(event) {
    // Verify the message is a height update
    if (event.data && event.data.setHeight) {
      const iframe = document.getElementById('exe_sum');
      if (iframe) {
        // Apply the new vertical vector immediately
        iframe.style.height = event.data.setHeight + 20 + 'px';
      }
    }
  }, false);
</script>
...
<iframe 
    ...
    id="key_f"
    width="100%" 
    style="border:none; width:100%; height: auto; min-height: 200px;" 
    frameborder="0" 
    scrolling="no">
</iframe>
...
<script>
  window.addEventListener('message', function(event) {
    // Verify the message is a height update
    if (event.data && event.data.setHeight) {
      const iframe = document.getElementById('key_f');
      if (iframe) {
        // Apply the new vertical vector immediately
        iframe.style.height = event.data.setHeight + 'px';
      }
    }
  }, false);
</script>
...
<script>
  window.addEventListener('message', function(event) {
    // Verify the message is a height update
    if (event.data && event.data.setHeight) {
      const iframe = document.getElementById('meth');
      if (iframe) {
        // Apply the new vertical vector immediately
        iframe.style.height = event.data.setHeight + 'px';
      }
